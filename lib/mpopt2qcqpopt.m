...
% mpopt2qcqpopt - Create/modify qcqps_master/miqcqps_master options struct from ``mpopt``.
% ::
%
...
%   MIQCQPS_MASTER or QCQPS_MASTER options struct.
%
...
%               OPT.alg for QCQPS_MASTER or MIQCQPS_MASTER. The first option
%               indicates that it should be taken from MPOPT.opf.ac.solver.
%
%   Output:
%       QCQPOPT : an options struct for use by QCQPS_MASTER or MIQCQPS_MASTER
%                 and friends
%
% See also qcqps_master, miqcqps_master, mpoption.
...
if strcmp(model, 'LP') || strcmp(model, 'QP') || strcmp(model, 'MILP') || strcmp(model, 'MIQP')
