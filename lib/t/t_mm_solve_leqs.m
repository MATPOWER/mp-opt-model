...
for k = 1:length(cfg)
    ...
    om = mp.opt_model;
    om.var.add('x', n1, zeros(n1, 1));
    om.lin.add(om.var, 'A', A1, b1, b1);
    if isempty(alg)
        opt = struct();
    else
    ...
    om = mp.opt_model;
    om.var.add('x', n3, zeros(n3, 1));
    om.lin.add(om.var, 'A', A3, b3, b3);
    if isempty(alg)
    ...
    om = mp.opt_model;
    om.var.add('x', n2, zeros(n2, 1));
    om.lin.add(om.var, 'A12', A2(1:2, :), b2(1:2), b2(1:2));
    om.lin.add(om.var, 'A3', A2(3, :), b2(3), b2(3));
...
t_is(om.var.get_soln(om.soln, 'x'), x, 14, [t 'x']);

t = 'om.lin.get_soln(om.var, om.soln, ''A12'') : ';
f12 = om.lin.get_soln(om.var, om.soln, 'A12');
t_is(f12{1}, f(1:2), 14, [t 'A12 * x - u12']);

t = 'om.lin.get_soln(om.var, om.soln, ''f'', ''A3'') : ';
g = om.lin.get_soln(om.var, om.soln, 'f', 'A3');
t_is(g, f(3), 14, [t 'A3 * x - u3']);

t = 'om.lin.get_soln(om.var, om.soln, ''l_Ax'', ''A3'') : ';
g = om.lin.get_soln(om.var, om.soln, 'l_Ax', 'A3');
