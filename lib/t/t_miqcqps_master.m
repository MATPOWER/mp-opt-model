function t_miqcqps_master(quiet)
% t_miqcqps_master - Tests of MIQP/MIQCQP solvers via miqcqps_master.
    ...
    if ~isempty(check{k}) && ~have_feature(check{k})
        ...
        if does_miqp(k)
            ...
            if does_nonconv(k)
                ...
                x0 = [8, 10, 0, 0]';
                [x, f, s, out, lam] = miqcqps_master(H, c, Q, B, lq, uq, A, l, u, xmin, xmax, x0, vtype, opt_f);
                t_is(s, 1, 12, [t 'success']);
                t_is(x, [8, 10, 2.0268, 7.1964]', 2, [t 'x']);
                t_is(f, -3434.2700, 2, [t 'f']);
            else
        ...
        if does_miqcqp(k)
            ...
            [x, f, s, out, lam] = miqcqps_master(H, c, Q, B, lq, uq, A, l, u, xmin, xmax, x0, vtype, opt_r);
            t_is(s, 1, 12, [t 'success']);
