function t_mm_solve_miqcqps(quiet)
% t_mm_solve_miqcqps - Tests of MIQP/MIQCQP solvers via mp.opt_model.solve.
...
for k = 1:length(algs)
    if ~isempty(check{k}) && ~have_feature(check{k})
        ...
        if does_miqp(k)
            ...
            vtype = 'IC';
            mm = mp.opt_model();            
            mm.var.add('x', 2, x0, xmin, xmax, vtype);
            mm.qdc.add(mm.var, 'cost', H, c);
            mm.qcn.add(mm.var, 'xQx', Q, B, lq, uq);
            ...
            if does_nonconv(k)
                ...
                vtype = 'IICC';
                mm = mp.opt_model(); 
                mm.var.add('x', 4, x0, xmin, xmax, vtype);
                mm.qdc.add(mm.var, 'cost', H, c);
                mm.qcn.add(mm.var, 'xQx', Q, B, lq, uq);
                mm.lin.add(mm.var, 'Ax', A, l, u);
                [x, f, s, out, lam] = mm.solve(opt);
        ...
        if does_miqcqp(k)
            ...
            if does_nonconv(k)
                ...
                vtype = 'IIICCCC';
                mm = mp.opt_model(); 
                mm.var.add('x', 7, x0, xmin, xmax, vtype);
                mm.qdc.add(mm.var, 'cost', H, c);
                mm.qcn.add(mm.var, 'xQx', Q, B, lq, uq);
                mm.lin.add(mm.var, 'Ax', A, l, u);
                [x, f, s, out, lam] = mm.solve(opt);
                ...
                mm.var.set_params('x','v0',ones(7,1));
                [x, f, s, out, lam] = mm.solve(opt_r);
                t_is(s, 1, 12, [t 'success']);
