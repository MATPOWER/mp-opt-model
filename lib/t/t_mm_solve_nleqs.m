    ...
    if ~isempty(check) && ~have_feature(check)
        ...
        switch alg
            case {'DEFAULT', 'NEWTON', 'FSOLVE', 'CORE-N'}
                ...
                x2 = [-2; 1; 3];
                om = mp.opt_model;
                om.var.add('x1', 2, x0_1);
                om.var.add('x2', 3, x0_2);
                om.nle.add(om.var, 'f', 2, @f1, [], {'x1'});
                om.lin.add(om.var, 'Ax_b', A2, b2, b2, {'x2'});
                [x, f, e, out, jac] = om.solve(opt);
                t_is(e, 1, 12, [t 'success']);
...
t = 'om.var.get_soln(om.soln, ''x1'') : ';
t_is(om.var.get_soln(om.soln, 'x1'), x(1:2), 14, [t 'x1']);

t = 'om.var.get_soln(om.soln, ''x'', ''x2'') : ';
t_is(om.var.get_soln(om.soln, 'x', 'x2'), x(3:5), 14, [t 'x2']);

t = 'om.lin.get_soln(om.var, om.soln, ''g'', ''Ax_b'') : ';
g = om.lin.get_soln(om.var, om.soln, 'g', 'Ax_b');
t_is(g{1}, f(3:5), 14, [t 'A * x - u']);
t_is(g{2}, f(3:5), 14, [t 'l - A * x']);

t = 'om.nle.get_soln(om.var, om.soln, true, ''f'') : ';
g = om.nle.get_soln(om.var, om.soln, true, 'f');
t_is(g, f(1:2), 14, [t 'f']);

t = 'om.nle.get_soln(om.var, om.soln, true, {''g'', ''dg''}, ''f'') : ';
[g, dg] = om.nle.get_soln(om.var, om.soln, true, {'g', 'dg'}, 'f');
t_is(g, f(1:2), 14, [t 'f']);
t_is(dg, jac(1:2, 1:2), 14, [t 'jac']);

t = 'parse_soln : ';
t_ok(om.has_parsed_soln(), [t 'has_parsed_soln() is true']);
t_is(om.var.soln.val.x1, om.var.get_soln(om.soln, 'x1'), 14, [t 'var.val.x1']);
t_is(om.var.soln.val.x2, om.var.get_soln(om.soln, 'x2'), 14, [t 'var.val.x2']);
