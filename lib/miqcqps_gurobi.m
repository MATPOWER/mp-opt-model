function [x, f, eflag, output, lambda] = miqcqps_gurobi(H, c, Q, B, lq, uq, A, l, u, xmin, xmax, x0, vtype, opt)
% miqcqps_gurobi - Mixed Integer Quadratically Constrained Quadratic Program Solver based on GUROBI.
% ::
%
%   [X, F, EXITFLAG, OUTPUT, LAMBDA] = ...
%       MIQCQPS_GUROBI(H, C, Q, B, LQ, UQ, A, L, U, XMIN, XMAX, X0, OPT)
%   [X, F, EXITFLAG, OUTPUT, LAMBDA] = MIQCQPS_GUROBI(PROBLEM)
%   A wrapper function providing a standardized interface for using
%   GUROBI to solve the following (possibly non-convex) MIQCQP (Mixed Integer
...
%
%       x = miqcqps_gurobi(H, c, Q, B, lq, uq)
%       x = miqcqps_gurobi(H, c, Q, B, lq, uq, A, l, u)
%       x = miqcqps_gurobi(H, c, Q, B, lq, uq, A, l, u, xmin, xmax)
%       x = miqcqps_gurobi(H, c, Q, B, lq, uq, A, l, u, xmin, xmax, x0)
%       x = miqcqps_gurobi(H, c, Q, B, lq, uq, A, l, u, xmin, xmax, x0, vtype)
%       x = miqcqps_gurobi(H, c, Q, B, lq, uq, A, l, u, xmin, xmax, x0, vtype, opt)
%       x = miqcqps_gurobi(problem), where problem is a struct with fields:
...
%       x = miqcqps_gurobi(...)
%       [x, f] = miqcqps_gurobi(...)
%       [x, f, exitflag] = miqcqps_gurobi(...)
%       [x, f, exitflag, output] = miqcqps_gurobi(...)
%       [x, f, exitflag, output, lambda] = miqcqps_gurobi(...)
%
...
if ~isempty(Q)
    [nq, ncolQ] = size(Q);
    if ~iscell(Q) || ncolQ ~= 1
        error('miqcqps_gurobi: Q must be column vector cell array.')
    end
    sizeQi  = cell2mat(cellfun(@(x)(size(x)), Q, 'UniformOutput', false));
    if any(sizeQi(:) - sizeQi(1))
        error('miqcqps_gurobi: All matrices Q{i}, i=1,...,%d must be square of the same size.', nq)
    end
elseif ~isempty(B)
    nq = size(B, 1);
else
...
if ~isempty(H)
    [nrowH, ncolH] = size(H);
    if nrowH ~= ncolH
        error('miqcqps_gurobi: H must be a square matrix.')
    end
    nx = nrowH;
else
    if ~isempty(c)
        ...
        if nq
            ...
            if ~isempty(B)
                ...
                if ~isempty(A)
                    ...
                    error('miqcqps_gurobi: inputs arguments H, c, Q, B, and A can not be all empty.')
                end
            end
        end
    end
...
if isempty(H) && isempty(c) && isempty(B) && isempty(A)
    error('miqcqps_gurobi: Problem is incomplete: H, c, B and A can not be all empty.')
end
if isempty(H)
    H = sparse(nx,nx);
end
if isempty(c)
    c = sparse(nx,1);
elseif length(c) ~= nx
    error('miqcqps_gurobi: Dimension of c (%d) must be iqual to the number of variables (%d).', length(c), nx)
end
...
if nq
    if isempty(B)
        ...
        if nrowB ~= nq || ncolB ~= nx
            error('miqcqps_gurobi: Dimension of B (%dx%d) should be number of quad constraints times number of variables (%dx%d).', nrowB, ncolB, nq, nx);
        end
    end
else
    Q = {};
    if ~isempty(B)
        [~, ncolB] = size(B);
        if ncolB ~= nx
            error('miqcqps_gurobi: The number of columns of matrix B (%d) must be equal to the number of variables (%d).', ncolB, nx);
        end
    else
        B = sparse(nq,nx);
        if ~isempty(lq) ||  ~isempty(uq)
            error('miqcqps_gurobi: No quadratic constraints were found. Inputs lq and uq should be empty.')
        end
    end
end
...
if ~isempty(A)
    [nlin, ncolA] = size(A);
    if ncolA ~= nx
        error('miqcqps_gurobi: The number of columns of matrix A (%d) must be equal to the number of variables (%d).', ncolA, nx);
    end
else
    nlin = 0;
    A = sparse(nlin,nx);
...
elseif length(uq) ~= size(B,1)
    error('miqcqps_gurobi: Dimension mismatch between uq, Q, and B.')
end
if isempty(lq)
    lq = -Inf(nq, 1);       %% ... unbounded below.
elseif length(lq) ~= size(B,1)
    error('miqcqps_gurobi: Dimension mismatch between lq, Q, and B.')
end
if isempty(u)           %% By default, linear inequalities are ...
    u = Inf(nlin, 1);       %% ... unbounded above and ...
elseif length(u) ~= nlin
    error('miqcqps_gurobi: Dimension of u (%d) must be iqual to the number of linear constraints (%d).', length(u), nlin')
end
if isempty(l)
    l = -Inf(nlin, 1);      %% ... unbounded below.
elseif length(l) ~= nlin
    error('miqcqps_gurobi: Dimension of l (%d) must be iqual to the number of linear constraints (%d).', length(l), nlin')
end
if isempty(xmin)        %% By default, optimization variables are ...
    xmin = -Inf(nx, 1);     %% ... unbounded below and ...
elseif length(xmin) ~= nx
    error('miqcqps_gurobi: Dimension of xmin (%d) must be iqual to the number of variables (%d).', length(xmin), nx')
end
if isempty(xmax)
    xmax = Inf(nx, 1);      %% ... unbounded above.
elseif length(xmax) ~= nx
    error('miqcqps_gurobi: Dimension of xmax (%d) must be iqual to the number of variables (%d).', length(xmax), nx')
end
if isempty(x0)
    x0 = zeros(nx, 1);
elseif length(x0) ~= nx
    error('miqcqps_gurobi: Dimension of x0 (%d) must be iqual to the number of variables (%d).', length(x0), nx')
end
